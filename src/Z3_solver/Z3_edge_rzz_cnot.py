...
class z3_edge_rzz_cnot:
    def __init__(self, num_qubit, layout=False, terms=[], G=False, I=False, rep = False):
        ...
        self.direct_to_undirected = {}
        for i, e in enumerate(layout):
            if e in self.layout_undirected:
                self.direct_to_undirected[i] = self.layout_undirected.index(e)
            else:
                self.direct_to_undirected[i] = self.layout_undirected.index((e[1],e[0]))
        self.rep = rep
    ...
    def validity_clauses(self, K):
        ...
        """
        for i in range(K): 
            all_gates = self.cnot[i]+self.rzz[i]
            self.solver.add(AtMost(*all_gates,1))
    ...
    def circuit_depth_encoding(self, K):
        ...
        for d in range(K):
            ...
            for i, (m,n) in enumerate(self.layout):
                self.solver.add(AtMost(*[And(Or(self.cnot[k][i],self.rzz[k][self.direct_to_undirected[i]]), self.depth_group[k][d]) for k in range(K)], 1))
                self.solver.add(self.depth_vector[d][i] == Or([And(Or(self.cnot[k][i],self.rzz[k][self.direct_to_undirected[i]]), self.depth_group[k][d]) for k in range(K)]))
            for q in range(self.bit_width):
                connected_cnot = []
                for i, (m,n) in enumerate(self.layout):
                    if m == q or n == q:
        ...
        for k in range(K+1):
            ...
            if k>0:
                ...
                #         qc.rzz(theta[n],self.layout_undirected[e][0],self.layout_undirected[e][1])
                    ...
                    matrix_rzz_row = []
                    for j in range(self.bit_width):
                        matrix_rzz_row.append(self.model[self.matrix_rzz[k-1][e][j]])
